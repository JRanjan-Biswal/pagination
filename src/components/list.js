  ...
  const changePerPage = (e, value) => {
    ...
    if (e.keyCode === 13) {
      if (!val) {
        ...
        alert(
          "Add a smaller Number! Adding such a bid number might become confusing. between 1-20"
        );
        return ;
      } else if (val < 1 || val === "") {
        alert("Add a bigger number. between 1-20")
        return;
      } 
      else {
        setCountPerPage(val);
  ...
  const changePageNumberFromInput = (e, value) => {
    ...
    if (e.keyCode === 13) {
      ...
      if (!val) {
        alert("Enter a valid Number");
        return ;
      } if(val > Math.floor(100 / parseInt(countPerPage))){
        alert("enter a number within the limit")
        return ;
      } else {
        setPageNumber(val);
      }
        ...
        <Stack spacing={2} className="paginationStack">
          ...
          <div style={{ margin: "1em auto" }}>
            <Pagination
              variant="outlined"
              shape="rounded"
              count={Math.floor(100 / parseInt(countPerPage))}
              onChange={(e, value) => {
